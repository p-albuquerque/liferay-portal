...
public class DefaultObjectEntryManagerImplTest
		...
		AssertUtils.assertFailure(
			...
			() -> _defaultObjectEntryManager.addObjectEntry(
				...
				new ObjectEntry() {
					{
						properties = HashMapBuilder.<String, Object>put(
							"r_oneToManyRelationshipName0_accountEntryId",
							accountEntry2.getAccountEntryId()
						).build();
					}
				},
					...
					new ObjectEntry() {
						{
							properties = HashMapBuilder.<String, Object>put(
								...
								StringBundler.concat(
									...
									index.getAndIncrement(), "_accountEntryId"),
								_addAccountEntry()
							).build();
						}
					},
	...
	private void _publishTree(
			boolean rootNodeAccountRestricted,
		...
		while (iterator.hasNext()) {
			...
			if (node.isRoot() && !rootNodeAccountRestricted) {
				index++;
			...
			objectRelationship =
				_objectRelationshipLocalService.addObjectRelationship(
					...
					LocalizedMapUtil.getLocalizedMap(
						RandomTestUtil.randomString()),
					"oneToManyRelationshipName" + index++, false,
					ObjectRelationshipConstants.TYPE_ONE_TO_MANY);
