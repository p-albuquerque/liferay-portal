...
public class BaseNotificationTypeTest {
	...
	protected void assertTerms(
		String termValues, Set<Map.Entry<String, String>> expectedValues) {

		List<String> termValueList = ListUtil.fromString(
			termValues, StringPool.SPACE);

		Object[] expectedTermValueArray = expectedValues.toArray();

		for (int i = 0; i < termValueList.size(); i++) {
			Map.Entry<String, String> expectedTermValue =
				(Map.Entry<String, String>)expectedTermValueArray[i];

			Assert.assertEquals(
				GetterUtil.getString(expectedTermValue.getValue()),
				termValueList.get(i));
		}
	}
